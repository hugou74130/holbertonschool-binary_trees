...
{ /* Début de la fonction qui prend un pointeur vers l'arbre et un pointeur de fonction en paramètres */
	...
	binary_tree_postorder(tree->left, func); /* Appel récursif pour parcourir d'abord le sous-arbre gauche */
	binary_tree_postorder(tree->right, func); /* Appel récursif pour parcourir ensuite le sous-arbre droit */
	func(tree->n); /* Appel de la fonction func avec la valeur du nœud courant (traitement du nœud en dernier) */
}